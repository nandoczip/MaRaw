...
f64 n_m_approx(f64 (*func)(f64, vec3_t, vec3_t), f64 x_0, f64 h, f64 epsilon, vec3_t a, vec3_t b) {
    ...
    for (i = 0, x = x_0; i < 10000 && f64_abs(func(x, a, b)) > epsilon; i++, x -= func(x, a, b) * h / (func(x + h, a, b) - func(x, a, b)));
...
vec2_t n_mm(f64 (*func)(vec2_t), f64 (*func_p)(vec2_t), vec2_t x_0, f64 epsilon) {
    ...
    for (x = x_0; f64_abs(func(x)) > epsilon; vec2_subeq(&x, vec2_mul((vec2_t) {1, 1}, func(x) / func_p(x)))) {vec2_printf(x);}
...
f64 diss(f64 x, vec3_t a) {
    return vec3_lensq(vec3_sub(a, g(x))) - 1;
}

f64 diss_p(f64 x, vec3_t a) {
    return -2 * vec3_dot(vec3_sub(a, g(x)), g_p(x));
}
    ...
    for (f64 t = t_0; t < t_1; t += 1 / fps) {
        for (ui32 y = 0; y < h; y++) {
            ...
            for (ui32 x = 0; x < w; x++) {
                ...
                for(x_0 = 2, i = 0; f64_abs(diss(x_0, hit)) > 1e-4 && i < 1000; x_0 -= diss(x_0, hit) / diss_p(x_0, hit), i++); 
